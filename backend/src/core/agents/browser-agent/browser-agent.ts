...
export class BrowserAgent {
  ...
  private onSuccess(runResult: RunResult) {
    this.reporter.success(
      `Manager agent completed successfully: ${runResult.answer}`,
    );
    this.runResult = runResult;
    this.isSuccess = true;
  }

  private onFailure({ reason }: RunResult) {
  ...
  async launch(startUrl: string, initialPrompt: string): Promise<RunResult> {
  ...
  setNewGoal(goal: string) {
    this.taskManager.setEndGoal(goal);
    this.runResult = null;
    this.isSuccess = false;
    this.isFailure = false;
  }

  async getInitialPageReport(url: string) {
    const vStartUrl = new VariableString(url, this.variables);
  ...
  async run(): Promise<RunResult> {
    return new Promise(async (resolve) => {
      this.reporter.loading('Starting browser agent');
      ...
      while (!this.isCompleted) {
        if (this.retries >= this.maxRetries) {
          return resolve({
            screenshot: null,
            extractedContentSummary: this.memoryLearnings.join(' ; '),
            status: 'failed',
            answer: this.result,
            stepCount: this.stepCount,
            reason:
              'Max number of retried reached. The agent was not able to complete the task.',
          });
        }
      ...
      /**
       * If the Browser Agent failed, then we return the failure reason immediately.
       */
      if (this.isFailure) {
  ...
  async defineNextTask(): Promise<Task> {
    ...
    const parser = new JsonOutputParser<BrowserAgentResponse>();
  ...
  private async executeAction(action: TaskAction) {
    ...
    switch (action.data.name) {
      ...
      case 'triggerResult':
        ...
        action.complete('trigger result has been called');
